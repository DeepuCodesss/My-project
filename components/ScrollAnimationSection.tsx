...
function Projects() {
  return <div className="projects-view"><div className="view-heading"><div><span className="eyebrow">/workspace/projects</span><h2>Things I&apos;ve shipped.</h2></div><span className="count-label">04 objects</span></div><div className="project-grid">{projects.map((project, index) => <motion.article key={project.name} className="project-card" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.07 }}><div className="project-card-top"><span className="project-index">0{index + 1}</span><span className="project-signal" style={{ backgroundColor: project.color }} /></div><span className="project-type">{project.type}</span><h3>{project.name}</h3><p>{project.description}</p><div className="stack-row">{project.stack.map(item => <span key={item}>{item}</span>)}</div><div className="card-links"><a href="#" onClick={(e) => e.preventDefault()}><Code2 size={13} /> Source</a><a href="#" onClick={(e) => e.preventDefault()}>Live demo <ChevronRight size={13} /></a></div></motion.article>)}</div></div>;
}
...
function Resume() { return <div className="resume-view"><div className="resume-paper"><div className="resume-name">DEEPAK<br /><span>KUMAR</span></div><div className="resume-role">Developer &amp; systems thinker</div><div className="resume-rule" /><div className="resume-cols"><div><small>PROFILE</small><p>I build clear, expressive digital products at the intersection of engineering and imagination.</p></div><div><small>SELECTED SKILLS</small><p>Frontend systems<br />Backend architecture<br />AI automation<br />Product design</p></div></div><div className="resume-footer">DEEPAK.K / 2026</div></div><div className="resume-actions"><span className="eyebrow">/documents/resume.pdf</span><h2>The longer version.</h2><p>A snapshot of the work behind the interface.</p><a className="deep-button" href="#" onClick={(e) => e.preventDefault()}><Download size={14} /> Download resume</a></div></div>; }

function Contact() { return <div className="contact-view"><span className="eyebrow">/inbox/new-message</span><h2>Let&apos;s build<br /><em>something together.</em></h2><p>Have an idea that deserves a thoughtful system around it? I&apos;d love to hear what you&apos;re working on.</p><div className="contact-links"><a href="mailto:hello@deepak.dev"><Mail size={15} /> Email me <ChevronRight size={14} /></a><a href="https://github.com" target="_blank" rel="noreferrer"><Code2 size={15} /> GitHub <ChevronRight size={14} /></a><a href="#" onClick={(e) => e.preventDefault()}><CircleUserRound size={15} /> LinkedIn <ChevronRight size={14} /></a></div></div>; }
